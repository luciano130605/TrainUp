...
import "./ajustes.css";
import { Check, Moon, Sun, RotateCcw, ArrowUpToLine, ArrowDownToLine, Bell, User, ChevronUp, ChevronDown } from 'lucide-react';
    ...
    return (
        <div className={`ajustes-dropdown ${open ? 'abierto' : ''}`}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <button className='action-sheet-icon ajustes-btn' role="switch" aria-checked={modoOscuro} onClick={onToggleModo}>
                    {modoOscuro ? <LunaIcon size={16} /> : <SolIcon size={16} />}
                </button>
                <button className='action-sheet-icon ajustes-btn' role="switch" onClick={irPerfil}>
                    <UserFillIcon size={16} />
                </button>
            </div>

            <h3 className="ajustes-dropdown-seccion-titulo">Color de acento</h3>
            ...
            <h3 className="ajustes-dropdown-seccion-titulo">Posición de las notificaciones</h3>
            <div className="ajustes-dropdown-toggle-fila">
                <button className={`btn-circle ${toasterPosition === 'top' ? 'select' : ''}`} onClick={() => onChangeToasterPosition('top')}><ChevronUp size={15} /></button>
                <button className={`btn-circle ${toasterPosition === 'bottom' ? 'select' : ''}`} onClick={() => onChangeToasterPosition('bottom')}><ChevronDown size={15} /></button>
            </div>
            ...
            <button className="add-exercise-btn home-hoy-btn ajustes" onClick={resetearAjustes}>
