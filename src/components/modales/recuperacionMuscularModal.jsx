...
export default function RecuperacionMuscularModal({ history, onClose }) {
    ...
    return (
        <div className="modal-overlay fixed flex justifyContentCenter" onClick={onClose}>
            <div className="action-sheet" onClick={e => e.stopPropagation()}>
                <div className="action-sheet-card progreso-card">
                    <h3 className="action-sheet-title">Recuperación muscular</h3>

                    {history.length === 0 ? (
                        <p className="action-sheet-desc">
                            Entrená al menos una vez para ver tu progreso acá.
                        </p>
                    ) : (
                        <>
                            <p className="action-sheet-desc">
                                Estimado según cuánto entrenaste cada músculo la última vez, comparado
                                con tu propio historial. No es un dato médico.
                            </p>

                            {pending.length > 0 && (
                                <div className="musc-scroll-row">
                                    {pending.map(r => (
                            ...
                            {recovered.length > 0 && (
                                <>
                                    <div className="musc-recuperados-titulo">Recuperados</div>
                                    <div className="musc-scroll-row">
                                        {recovered.map(r => (
